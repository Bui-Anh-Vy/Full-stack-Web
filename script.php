<?php 
	...
	if(isset($_POST['submit'])){
		...
		$new_message = array(
			...
			"retype_passowrd" => $_POST['retype_password'],
			"email" => $_POST['email'],
		...
		if(filesize("userdata.json") == 0){
			...
			$old_records = json_decode(file_get_contents("userdata.json"));
		...
		// Now our last step is to store the data to the file (userdata.json).
		if(!file_put_contents("userdata.json", json_encode($data_to_save, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE), LOCK_EX)){
